	...
	, NodeToKeepStill(InNodeToKeepStill)
{
	...
	bCenterBranches = UBASettings::Get().bCenterBranchesForParameters;
	NumRequiredBranches = UBASettings::Get().NumRequiredBranchesForParameters;
...
void FEdGraphParameterFormatter::FormatNode(UEdGraphNode* InNode)
{
	...
	if (FBAUtils::IsNodeImpure(RootNode))
	{
		...
		{
			float Baseline = FBAUtils::GetPinPos(GraphHandler, Pin).Y + UBASettings::Get().ParameterVerticalPinSpacing;
...
{
	EBAParameterFormattingStyle FormattingStyleToUse = UBASettings::Get().ParameterStyle;
	...
	if (UBASettings::Get().bLimitHelixingHeight)
	{
		float TotalHeight = 0;
		for (UEdGraphNode* Node : GatheredInputNodes)
		{
			if (Node != RootNode)
			{
				...
				if (NodeHeight > UBASettings::Get().SingleNodeMaxHeight)
				{
					return false;
				}
		...
		if (TotalHeight > UBASettings::Get().HelixingHeightMax)
		{
			return false;
		}
	}
	...
	for (EEdGraphPinDirection Direction : { EGPD_Input, EGPD_Output })
	{
		...
		for (const FPinLink& Link : FBAUtils::GetPinLinks(CurrentNode, Direction))
		{
			...
			if (!(bFormatWithHelixing && Direction == EGPD_Input) && LocalChildren.Num() > 0 && LastLinked != nullptr)
			{
				...
				const float PinPos = GraphHandler->GetPinY(LastLinked) + UBASettings::Get().ParameterVerticalPinSpacing;
...
void FEdGraphParameterFormatter::MoveBelowBaseline(TSet<UEdGraphNode*> Nodes, const float Baseline)
{
	...
	const FSlateRect Bounds
		= UBASettings::Get().bApplyCommentPadding
