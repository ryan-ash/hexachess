...
void FSimpleFormatter::FormatNode(UEdGraphNode* Node)
{
	...
	// UE_LOG(LogBlueprintAssist, Warning, TEXT("Simple Formatter root node %s"), *FBAUtils::GetNodeName(RootNode));
	// for (UEdGraphNode* NodesToFormat : GetFormatterParameters().NodesToFormat)
	// {
	// 	UE_LOG(LogTemp, Warning, TEXT("\t%s"), *FBAUtils::GetNodeName(NodesToFormat));
	// }
...
bool FSimpleFormatter::ShouldIgnoreComment(TSharedPtr<FBACommentContainsNode> ContainsNode)
{
	...
	return NodesUnderComment.Difference(FormattedNodes).Num() > 0;
}
