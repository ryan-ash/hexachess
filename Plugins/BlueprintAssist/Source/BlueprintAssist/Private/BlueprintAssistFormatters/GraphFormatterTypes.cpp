...
bool FFormatterConnectionValidator::CheckChanged(const TArray<UEdGraphNode*>& Nodes)
{
	...
	if (DifferentNodes.Num())
	{
		...
		{
			UE_LOG(LogBlueprintAssist, Warning, TEXT("Formatting has caused node to be removed or added: %s. Please report this issue!"), *FBAUtils::GetNodeName(Node));
		}
	...
	for (auto& Node : OldNodes)
	{
		...
		if (DifferentLinks.Num())
		{
			...
			{
				UE_LOG(LogBlueprintAssist, Warning, TEXT("Formatting has caused node connections to change %s. Please report this issue!"), *Difference.ToString());
			}
