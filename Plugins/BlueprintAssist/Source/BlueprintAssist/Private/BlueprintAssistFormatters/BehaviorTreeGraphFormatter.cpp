...
void FBehaviorTreeGraphFormatter::FormatNodes(UEdGraphNode* InNode, TSet<UEdGraphNode*>& VisitedNodes) const
{
	...
	TArray<UEdGraphPin*> LinkedOutPins;
	// FBAUtils::GetLinkedPins(InNode, EGPD_Output) <- This doesn't work for the Dialogue plugin because it hides hidden pins!
	for (UEdGraphPin* Pin : InNode->Pins)
	{
		if (Pin->LinkedTo.Num() && Pin->Direction == EGPD_Output)
		{
			LinkedOutPins.Add(Pin);
		}
	}
